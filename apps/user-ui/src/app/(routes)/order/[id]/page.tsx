...
const OrderDetailsPage = () => {
    ...
    if(!order) {
        return <p className="text-center mt-4 text-sm text-red-500">Order not found.</p>
    }
                ...
                <div className="flex items-center justify-between text-xs font-medium text-gray-500 mb-2">
                    {[
                        ...
                        const passed = idx <= [
                            ...
                            "out for delivery",
                            "delivered"
                        ].findIndex(status => status.toLowerCase() === (order.deliveryStatus || "processing").toLowerCase());
                    ...
                    {[
                        ...
                        return (
                            <div key={step} className={`flex-1 flex items-center`}>
                                <div className={`w-4 h-4 rounded-full ${isReached ? "bg-blue-600" : "bg-gray-400"}`} />
                                {idx !== 4 && (
                                    <div className={`flex-1 h-1 ${isReached ? "bg-blue-600" : "bg-gray-400"}`}></div>
                                )}
                            </div>
                        )
                    })}
