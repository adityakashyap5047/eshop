                ...
                render={({ field }) => (
                    <div className="flex gap-3 flex-wrap">
                        {[...defaultColors, ...customColors].map((color) => {
                            ...
                            const isLightColor = ["#ffffff", "#fff00"].includes(color);
                            const isDarkColor = ["#000000", "#00000"].includes(color);
                            ...
                            return (
                                <button type="button" key={color}
                                    ...
                                    className={`w-7 h-7 p-2 rounded-md my-1 flex items-center justify-center border-2 transition ${isSelected ? 'scale-110 border-white' : "border-transparent"} ${isLightColor ? 'border-gray-600' : ''} ${isDarkColor ? '!border-gray-800' : ''}`}
